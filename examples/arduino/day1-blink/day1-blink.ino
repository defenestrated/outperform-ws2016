...
/*
   ...
   it's an integer, or "int" for short,
   which means that it has to be a whole number -- no decimal places.
   then, by saying maggie = 13, that means maggie now holds the number 13.
   so in the future, anytime we say "maggie," arduino sees the *value*
   held by the variable named "maggie" -- in this case, the number 13.
*/

int ontime = 100;
int offtime = 250;
/*
   same thing for "ontime" and "offtime" --
   note that you can't have spaces in variable or function names.
*/


void setup() {
  /*
     "setup" runs once when arduino starts up.
     it's a good place to, well, set everything up.
     don't worry about the "void" part, we'll get into that later.

     first, you need to use the function "pinMode":
     this lets you decide whether a certain pin will be used for input or output --
     meaning, whether you are asking it for data (whether a button is
     pressed, for example), or sending out data (like turning on an LED).
  */

  pinMode(maggie, OUTPUT);
  /*
     the syntax of pinMode (syntax is the format you
     need to use to get a piece of code to work) is:

     pinMode([which pin you want to set], [whether it's INPUT or OUTPUT]);
